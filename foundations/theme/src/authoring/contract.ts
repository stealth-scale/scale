...
import { type SemanticTokens, type ThemeVariant } from "#pandacss.ts";

/**
 * Lists the twelve roles a palette fills, each named for its use. A dotted name nests under its
...
/**
 * Lists the hue ramps the foundation draws, each of which fills the twelve roles.
 */
export const HUES = [
  "blue",
  "cyan",
...
/**
 * Lists the semantic palettes a recipe names an intent with, each filled by reference to a hue.
 */
 ...
 */
export const FOREGROUNDS = ["DEFAULT", "muted", "subtle", "inverted", "disabled", "link"] as const;
...
/**
 * Nests the twelve roles the way the compiler reads them: a dotted role under its group with the
...
/**
 * Describes every color a root theme states.
 */
...
/**
 * Describes the switchable half of a theme once the contract is filled.
 */
export type ContractedVariant = Record<"semanticTokens", ThemeTokens> & ThemeVariant;

/**
 * Checks a theme's switchable values against the contract and returns them unchanged.
 *
 * @remarks
 *   The check is the type of the parameter. A theme that leaves a role out draws a control with no
 *   background under one palette, and nothing at run time would say so.
 */
export function contract<Variant extends ContractedVariant>(variant: Variant): Variant {
  return variant;
}
